...
def append_summary(report_path: Path, trace_path: Path, debug_dir: Path) -> dict[str, Any]:
    ...
    report["alerts"] = [item for item in report.get("alerts", []) if not (isinstance(item, dict) and item.get("metric") == "mixed_subject_violation_rate")]
    report["bug_classifications"] = [item for item in report.get("bug_classifications", []) if not (isinstance(item, dict) and item.get("code") == "BUG_MIXED_SUBJECT_LIKELY")]
    if unblocked:
        report.setdefault("alerts", []).append({"metric": "mixed_subject_violation_rate", "severity": "hard_block", "reason": "one or more drafts contain multiple significant visible tracks without review-required blocking"})
        report.setdefault("bug_classifications", []).append({"code": "BUG_MIXED_SUBJECT_LIKELY", "evidence": f"{len(unblocked)} unblocked mixed-subject source-window(s)"})
...
def main() -> int:
    ...
    print("review-window summary " f"detected={metrics.get('mixed_subject_likely_window_count', 0)} " f"blocked={metrics.get('mixed_subject_blocked_window_count', 0)} " f"unblocked={metrics.get('mixed_subject_unblocked_window_count', 0)}")
    return 0
