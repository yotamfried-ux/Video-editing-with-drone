...
const RUN_STATUS_LABEL: Record<string, string> = {
  ...
  no_input: 'No input', dispatch_failed: 'Dispatch failed',
};
const UPLOAD_STATUS_LABEL: Record<UploadItemStatus, string> = {
  queued: 'Queued', initializing: 'Preparing', uploading: 'Uploading', verified: 'Verified', failed: 'Failed',
};
...
function displayProgressForRun(run: PipelineRun | null): number {
  ...
  return ['succeeded', 'failed', 'dispatch_failed', 'no_input'].includes(run.status) ? 1 : 0;
}
...
export default function PipelineScreen() {
  ...
  const {
    status, latestRun, globalLiveStale, globalLiveStaleReason,
  ...
  const handleOperatorError = (error: unknown) => {
    ...
    } else Alert.alert('Failed', message);
  };
  ...
  const confirmReset = () => Alert.alert('Reset and rerun', 'Choose reset scope:', [
    { text: 'Cancel', style: 'cancel' },
  ...
  const requestUploadSession = async (item: UploadFileState, batchId: string, sourceSizeBytes: number): Promise<UploadSession> => {
    const uploadInit = await operatorFetch<UploadInit>('/api/operator/upload', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        filename: item.filename, mimeType: item.mimeType, batch_id: batchId,
        upload_mode: 'multipart_resumable', client_upload_id: item.id,
  ...
  const runUploadQueue = async (items: UploadFileState[], startingBatchId: string | null) => {
    ...
    const concurrency = items.some((item) => item.externalSource) ? EXTERNAL_STORAGE_UPLOAD_CONCURRENCY_LIMIT : UPLOAD_CONCURRENCY_LIMIT;
  ...
  const retryUploadItem = async (item: UploadFileState) => {
    try { await uploadItemWithRetries(item, item.batch_id ?? activeBatchId ?? newClientBatchId()); }
    catch (error) { handleOperatorError(error); }
  };
      ...
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Discard', style: 'destructive', onPress: async () => {
          try {
            ...
            const record = restored?.uploads.find((upload) => upload.clientUploadId === item.id);
            if (record) await abortPersistedMultipartUpload(record);
            const remaining = uploadItems.filter((upload) => upload.id !== item.id);
            if (!remaining.length && item.batch_id) {
              await clearPersistedMultipartBatch(item.batch_id);
              setActiveBatchId(null);
            }
  ...
  const retryAllFailedUploads = async () => {
    ...
    try {
      ...
      if (failedAgain) Alert.alert('Some uploads still failing', `${failedAgain} of ${failedItems.length} files failed again. Check the connection and retry when stable.`);
      else Alert.alert('Uploads verified', `${failedItems.length} previously failed files resumed and verified.`);
    } finally { setRetryingFailedUploads(false); }
  };
  const uploadSelectedItems = async (items: UploadFileState[]) => {
    if (items.length > MAX_UPLOAD_BATCH_FILES) {
      Alert.alert('Too many videos', `Select at most ${MAX_UPLOAD_BATCH_FILES} videos for one batch.`); return;
    }
    if (uploadItems.some((item) => item.status !== 'verified')) {
      Alert.alert('Resolve current uploads', 'Retry or complete every failed upload before adding more footage to this batch.'); return;
    }
    setUploadItems(items);
    const { stableBatchId, results } = await runUploadQueue(items, activeBatchId);
    const failed = results.filter((result) => result.status === 'rejected').length;
    if (failed) {
      Alert.alert('Some uploads failed', `${failed} of ${items.length} files failed after part-level retries. Use Retry all failed to resume only missing parts.`); return;
    }
  ...
  const uploadFootage = async () => {
    ...
    if (!permission.granted) { Alert.alert('Permission required', 'Please allow access to your media library.'); return; }
    const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: 'videos', allowsMultipleSelection: true, videoMaxDuration: 7200 });
    if (result.canceled || !result.assets?.length) return;
    const items: UploadFileState[] = result.assets.map((asset, index) => {
      const filename = selectedAssetFilename(asset, index);
      return {
        id: `${Date.now()}_${index}_${filename}`, uri: asset.uri, filename,
        mimeType: selectedAssetMimeType(asset), sourceSizeBytes: selectedAssetSize(asset),
        progress: 0, status: 'queued', batch_id: activeBatchId, error: null, externalSource: false,
      };
    });
    await uploadSelectedItems(items);
  };
  const toggleExternalCandidate = (id: string) => setExternalCandidates((candidates) => candidates.map((candidate) => candidate.id === id ? { ...candidate, selected: !candidate.selected } : candidate));
  const uploadSelectedExternalVideos = async () => {
    const selected = externalCandidates.filter((candidate) => candidate.selected);
    if (!selected.length) { Alert.alert('Select videos', 'Choose at least one video from the folder before uploading.'); return; }
  ...
  const uploadExternalStorageFolder = async () => {
    if (Platform.OS !== 'android') { Alert.alert('Android only', 'Direct SD / USB video selection is currently available on Android.'); return; }
    ...
    try {
      ...
      if (!videos.length) {
        setExternalCandidates([]);
        Alert.alert('No videos found', 'Choose the SD / USB folder that directly contains the video clips.'); return;
      }
      setExternalCandidates(videos.map((document, index) => ({
        id: `${permission.directoryUri}_${index}_${document.filename}`,
        uri: document.uri, filename: document.filename,
        mimeType: mimeTypeForFilename(document.filename), selected: false,
      })));
    } catch (error) { handleOperatorError(error); }
    finally { setSelectingExternalStorage(false); }
  };
      ...
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={{ gap: Spacing.md, paddingBottom: Spacing.xl }}>
          ...
          <Text variant="caption" color={statusError ? Colors.danger : Colors.textSecondary}>
            {statusError ? `Status unavailable: ${statusError}` : statusLoading ? 'Loading global live status...' : `Global live status · polls every 5s${status?.updated_at ? ` · updated ${new Date(status.updated_at).toLocaleTimeString()}` : ''}`}
          </Text>
          <Card bordered style={{ gap: Spacing.md }}>
            <View style={{ gap: Spacing.xs }}>
              <Text variant="title">Global live progress</Text>
              <Text variant="caption" color={Colors.textSecondary}>Upload all footage for a batch first. Run the pipeline only when every selected upload is verified.</Text>
                ...
                {externalCandidates.length > 0 && (
                  <View style={styles.externalSelectionPanel}>
                    ...
                    {externalCandidates.map((candidate) => (
                      <Pressable key={candidate.id} onPress={() => toggleExternalCandidate(candidate.id)} disabled={busy} accessibilityRole="checkbox" accessibilityState={{ checked: candidate.selected, disabled: busy }} style={({ pressed }) => [styles.externalSelectionRow, candidate.selected && styles.externalSelectionRowSelected, pressed && !busy && { opacity: 0.75 }]}>
                  ...
                  <View key={item.id} style={styles.uploadRow}>
                    ...
                    {item.status === 'failed' && (
                      <View style={styles.uploadActions}>
                        <Button label="Resume" onPress={() => retryUploadItem(item)} disabled={busy} variant="ghost" style={{ height: 36 }} />
                        <Button label="Discard" onPress={() => discardUploadItem(item)} disabled={busy} variant="ghost" style={{ height: 36 }} />
                      </View>
                    )}
                  </View>
                ))}
          ...
          <Card bordered style={{ gap: Spacing.sm }}>
            ...
            {STAGES.map((stage) => {
              ...
              return <View key={stage} style={styles.stageRow}><View style={[styles.dot, done && { backgroundColor: Colors.success }, isCurrent && { backgroundColor: Colors.accent }]} /><Text variant="body" color={isCurrent ? Colors.textPrimary : Colors.textSecondary}>{stage.toUpperCase()}</Text></View>;
            })}
          </Card>
          {requests.length > 0 && (
            <Card bordered style={{ gap: Spacing.sm }}>
              ...
              {requests.map((request) => (
                <View key={request.id} style={{ gap: 2 }}>
                  <View style={styles.metaRow}><Text variant="caption" color={Colors.textPrimary} numberOfLines={1} style={{ flex: 1 }}>{request.draft_name || request.id.slice(0, 8)}</Text><Text variant="caption" color={Colors.accent}>{STATUS_LABEL[request.status] ?? request.status}</Text></View>
          ...
          {Object.keys(meta).length > 0 && (
            <Card bordered style={{ gap: Spacing.xs }}>
              ...
              {Object.entries(meta).map(([key, value]) => <View key={key} style={styles.metaRow}><Text variant="caption" color={Colors.textSecondary}>{key}</Text><Text variant="caption" color={Colors.accent}>{String(value)}</Text></View>)}
            </Card>
          )}
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  uploadRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, borderWidth: 1, borderColor: Colors.cardBorder, borderRadius: 8, padding: Spacing.sm },
  uploadActions: { gap: Spacing.xs, alignItems: 'flex-end' },
  externalSelectionPanel: { gap: Spacing.sm, borderWidth: 1, borderColor: Colors.cardBorder, borderRadius: 8, padding: Spacing.sm },
  externalSelectionRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, borderWidth: 1, borderColor: Colors.cardBorder, borderRadius: 8, padding: Spacing.sm },
  externalSelectionRowSelected: { borderColor: Colors.accent },
  selectionIndicator: { width: 24, height: 24, borderRadius: 6, borderWidth: 1, borderColor: Colors.cardBorder, alignItems: 'center', justifyContent: 'center' },
  selectionIndicatorSelected: { backgroundColor: Colors.accent, borderColor: Colors.accent },
  staleNotice: { gap: Spacing.xs, borderLeftWidth: 3, borderLeftColor: Colors.accent, paddingLeft: Spacing.sm },
});
