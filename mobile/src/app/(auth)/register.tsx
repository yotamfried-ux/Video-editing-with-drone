...
import { FaceUploadStep } from '@/features/auth/components/FaceUploadStep';
import { useRegistration } from '@/features/auth/hooks/useRegistration';
import { Colors, Spacing } from '@/shared/constants/theme';

const STEPS = ['Account', 'Profile', 'Face ID'] as const;
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
        ...
        {reg.step === 'credentials' && (
          <View style={styles.form}>
            ...
            <TextInput placeholder="Email" placeholderTextColor={Colors.textSecondary} value={reg.email} onChangeText={reg.setEmail} keyboardType="email-address" autoCapitalize="none" style={styles.input} />
            <TextInput placeholder="Password (min 6 chars)" placeholderTextColor={Colors.textSecondary} value={reg.password} onChangeText={reg.setPassword} secureTextEntry style={styles.input} />
            ...
            <Button label="Already have an account?" onPress={() => router.push('/(auth)/login')} variant="ghost" style={{ marginTop: Spacing.sm }} />
          </View>
        )}
        ...
        {reg.step === 'profile' && (
          <View style={styles.form}>
            ...
            <Spacer size={Spacing.xl} />
            <TextInput placeholder="Full name" placeholderTextColor={Colors.textSecondary} value={reg.name} onChangeText={reg.setName} style={styles.input} />
            {reg.error && <Text variant="caption" color={Colors.danger}>{reg.error}</Text>}
            <Spacer size={Spacing.md} />
            <Button label="Continue" onPress={reg.submitProfile} loading={reg.loading} />
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    borderWidth: 1, borderColor: '#2D3F55', fontSize: 15,
  },
});
