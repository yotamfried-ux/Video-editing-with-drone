...
// POST /api/operator/pipeline/start — operator triggers a tracked pipeline run from the app.
export async function POST(req: NextRequest) {
  ...
  if (insertError || !run) {
    return NextResponse.json(
      { error: insertError?.message ?? 'Could not create pipeline run' },
      { status: 500 },
    );
  }
  ...
  const res = await fetch(`https://api.github.com/repos/${repo}/dispatches`, {
    ...
    body: JSON.stringify({
      ...
      client_payload: { pipeline_run_id: run.id, source: 'manual' },
    }),
  });
