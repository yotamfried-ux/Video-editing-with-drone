...
function isActiveDiscoverStatus(status: string | null) {
  return ACTIVE_DISCOVER_STATUSES.includes(status ?? '');
}
...
export async function GET(req: NextRequest) {
  ...
  const reels = data ?? [];
  const now = Date.now();
  const activeReels = reels.filter((reel) => isActiveDiscoverStatus(reel.status));
  const missingExpiryCount = activeReels.filter((reel) => !reel.expires_at).length;
  const expiredActiveCount = activeReels.filter((reel) => {
    if (!reel.expires_at) return false;
    const expiresAt = new Date(reel.expires_at).getTime();
  ...
  return NextResponse.json({
    ...
    activeReelCount: activeReels.length,
    missingExpiryCount,
    expiredActiveCount,
    sessions: groupSessions(reels),
    reels,
  });
}
