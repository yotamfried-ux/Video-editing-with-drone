...
# One reel per athlete, up to the Instagram Reels 90s ceiling. Splitting at 30s
# scattered each athlete's session across 3-4 drafts; the product target is a
# single consolidated reel ≤90s.
TARGET_REEL_MAX = 85

# Encoding: intermediate clips get re-encoded again at compile, so they must be
# visually transparent (CRF 14) or the double generation visibly degrades the output.
CLIP_CRF  = "14"
REEL_CRF  = "16"
X264_PRESET = "medium"
# Lanczos resampling for every scale step — portrait conversion upscales a
# 1080p landscape source ×1.78, where bicubic (default) visibly softens.
_SCALE_FLAGS = ":flags=lanczos"


def _clamp(start: float, end: float, video_path: str) -> tuple[float, float] | None:
    """Clamp event window to the source duration.

    Returns None when the event lies (almost) entirely beyond the source —
    those are hallucinated timestamps (Gemini sometimes reports times past the
    chunk length). Clamping them used to crush every such event into the same
    final 4 seconds of the video, producing several identical clips per reel.
    """
    total = _get_duration(video_path)
    if start >= total - 2.0:
        logger.warning("⚠️ Event [%.1f,%.1f] beyond source end (%.1fs) — dropped", start, end, total)
        return None
...
def _sanitize_events(events: list[dict], video_path: str) -> list[dict]:
    """Validate event windows against the real source duration and de-duplicate.

    - Drops events that start beyond the source end (hallucinated timestamps).
    - Trims ends that overrun the source.
    - Drops events whose window overlaps an already-kept higher-scored event by
      >40% — the same physical moment must never appear twice in one reel.
    """
    try:
        total = _get_duration(video_path)
    except Exception:
    ...
    for ev in events:
        ...
        if s >= total - 2.0:
            logger.warning("⚠️ Dropping event '%s' [%.1f,%.1f] — beyond source end (%.1fs)",
                           ev.get("type", "?"), s, e, total)
            continue
        e = min(e, total)
        if e - s < 4.0:
            logger.info("Dropping event '%s' — only %.1fs remain inside source", ev.get("type", "?"), e - s)
            continue
        valid.append({**ev, "end": round(e, 2)})

    valid.sort(key=lambda ev: ev.get("score", 0), reverse=True)
    kept: list[dict] = []
    for ev in valid:
        s, e = ev["start"], ev["end"]
        dur  = e - s
        ...
        for k in kept:
            ...
            if overlap > 0.4 * min(dur, k["end"] - k["start"]):
                logger.info("Dropping duplicate event '%s' [%.1f,%.1f] — overlaps kept [%.1f,%.1f]",
                            ev.get("type", "?"), s, e, k["start"], k["end"])
                dup = True
                break
        if not dup:
            kept.append(ev)
...
def _enforce_single_slowmo(events: list[dict]) -> list[dict]:
    """Enforce editorial rule: at most ONE slowmo per reel — the climax (last event),
    and only when it earned it.

    Slow-motion is an emphasis tool, not a default: it applies only when Gemini
    flagged the moment as slowmo-worthy AND the score is >= 8 (a genuinely
    impressive move). All non-climax events get slowmo stripped. The climax is
    marked with _is_climax so cut_clip() knows which clip may run long.
    ...
    for i, ev in enumerate(events):
        ...
        edit["slowmo"] = (
            i == last
            and bool(edit.get("slowmo", False))
            and int(ev.get("score", 0)) >= 8
        )
        result.append({**ev, "edit": edit, "_is_climax": i == last})
    return result
...
def cut_clip(
    ...
    - ללא אודיו
    """
    _clamped = _clamp(event["start"], event["end"], video_path)
    if _clamped is None:
        return None
    start, end = _clamped
    input_dur  = end - start
    ...
    # Note: type hints may BLOCK slowmo (wipeout/crash) but never force it —
    # slow-motion is reserved for moments Gemini explicitly flagged as worthy.
    ...
    # Slow-mo as brief emphasis: slow only ~2.5s of source around the action
    # peak. Spreading slow-mo across 40-50% of the clip (the old behaviour) made
    # reels feel stretched and unnatural — emphasis only works when it is short.
    # Interpolated slow-mo is gentler (1.5×): at 2× a 24fps source drops to an
    # effective 12fps and optical flow turns the athlete to mush.
    if use_slowmo:
        slow_factor = 1.5 if slowmo_interp else 2.0
        sm_frac     = round(min(0.30, 2.5 / max(input_dur, 0.1)), 3)
        sm_pts_expr = f"{slow_factor}*(PTS-STARTPTS)"
    else:
    ...
    half_w  = REEL_W // 2
    fg_wide = (
        f"scale=iw*{REEL_H}/ih:{REEL_H}{_SCALE_FLAGS},"
    ...
    if applied_zoom > 1.05:
        ...
        fg_zoom = (
            ...
            f"scale={REEL_W}:{REEL_H}{_SCALE_FLAGS}"
        )
    else:
...
def compile_multi_source_reel(appearances: list[dict], sport: str = "",
    ...
    # Sanitize per-source first: drops hallucinated timestamps and deduplicates moments
    # within each clip so a bad source doesn't corrupt the cross-source reel.
    all_events: list[dict] = []
    for app in appearances:
        sanitized = _sanitize_events(app["events"], app["path"])
        for ev in sanitized:
...
def create_reel(video_path: str, events: list[dict], sport: str = "",
    ...
    # Sanitize before everything: drops hallucinated timestamps and deduplicates
    # overlapping moments so partitioning + clip cutting work on clean input.
    events     = _sanitize_events(events, video_path)
    if not events:
        print(f"⚠️ '{Path(video_path).name}': all events invalid after sanitization — skipping")
        return []
    ...
    for part_idx, part_events in enumerate(partitions):
        ...
        for _i, _ev in enumerate(ordered):
            _desc = str(_ev.get("description", ""))[:80]
            _sm   = "🐢slow" if _ev.get("edit", {}).get("slowmo") else ""
            _zm   = f"×{_ev.get('edit',{}).get('zoom',1.0):.1f}zoom" if _ev.get("edit", {}).get("zoom", 1.0) > 1.05 else ""
            print(f"   [{_i+1}] score={_ev.get('score','?')} {_ev.get('type','?')} "
                  f"{_ev.get('start',0):.1f}→{_ev.get('end',0):.1f}s {_sm}{_zm} — {_desc}")
