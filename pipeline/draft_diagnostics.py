...
def build_diagnostic_artifact(draft_name: str, sport: str, events: list[dict[str, Any]], source_quality: dict[str, Any], final_upload_key: str | None = None) -> dict[str, Any]:
    ...
    for idx, event in enumerate(events):
        ...
        identity_members.append({"event_id": eid, "person_id": event.get("person_id") or event.get("athlete_id"), "identity_confidence": event.get("identity_confidence"), "identity_mismatch": event.get("identity_mismatch")})
        ordered.append({"order": idx, "event_id": eid, "type": event.get("type", ""), "score": event.get("score"), "start": event.get("start"), "end": event.get("end"), "final_cut_start": event.get("final_cut_start"), "final_cut_end": event.get("final_cut_end"), "cut_adjustment_reason": event.get("cut_adjustment_reason"), "is_teaser": bool(event.get("_teaser")), "is_climax": bool(event.get("_is_climax"))})
