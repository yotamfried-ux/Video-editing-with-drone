...
| Pipeline | `pipeline/`, `scripts/run_tracked.py`, `run.py` | Python, Gemini, FFmpeg, storage, Supabase, and QA/editing logic. Runs in GitHub Actions or locally for development. |
| Delivery | `deliver.py`, `services/delivery.py`, `.github/workflows/deliver.yml` | Moves approved reels through delivery/payment handoff and records durable delivery runs. |
...
| Database and diagnostics | `supabase/`, `scripts/run_pipeline_with_diagnostics.sh` | Migrations, tracking tables, run evidence, coverage reports, Discover smoke SQL, and payment/pipeline state. |
...
Local pipeline runs are for development and diagnostics. Production-style operator runs should be dispatched through the API and GitHub Actions so `pipeline_runs`, `pipeline_status`, storage moves, product-contract evidence, and app status screens stay aligned.
...
- `docs/audit/personal-publishable-reel-completion-plan-20260717.md` — business vision, official-source research, implementation stages, and acceptance gates.
...
The pipeline must be evaluated against the Product vision above, not only against workflow success. Keep future PRs narrow, update the relevant audit when a gap changes status, and do not close a footage-level gap without a real run and visual review.
