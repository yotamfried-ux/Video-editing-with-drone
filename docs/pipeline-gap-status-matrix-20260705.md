...
| PQ-002 | Identity clustering uses stable perception evidence and fails safe. | Identity fail-safe and real identity gate exist; #109 adds deterministic athlete IDs/merge evidence; #110 adds same-window multi-person review-required diagnostics, but production tracker validation is still missing. | #90, #99, #106, #109, #110 | Contract/CI + prior real run still failing | Partial | Validate same-window identity behavior on the next real run and build tracker-backed evidence. |
...
| PQ-009 | Critical QA defects do not look like normal approvals. | QA diagnostics/fail-closed exist, and #110 adds `MULTI_PERSON_CLIP` / `IDENTITY_UNCERTAIN` as blocking review-required defects; UI still shows normal Approve for QA-labelled drafts. | #97, #103, #104, #105, #110 | Contract/CI + real UI gap | Partial | Add UI gating and blocking-reason visibility. |
| PQ-010 | Draft output is reconstructable from metadata. | Draft diagnostic artifact exists; #108 adds candidate ledger metadata; #110 persists `multi_person_clip_gate`; latest real run still had no downloadable Actions artifacts and UI metadata is still insufficient. | #98, #108, #110 | Contract/CI | Partial | Verify candidate ledger and multi-person gate in real artifacts and expose metadata in operator UI. |
| REAL-ID-001 | Mixed athletes do not reach normal review drafts. | Real identity gate exists; #109 adds run-level athlete IDs; #110 flags same-window multi-person drafts as review-required; tracker-backed proof is still missing. | #99, #106, #109, #110 | Contract/CI + prior real run still failing | Partial | Validate against real outputs and add production tracker source-of-truth. |
...
| REAL-ID-003 | Similar surfers inside source windows are not silently merged. | Identity uncertainty can be surfaced; #109 avoids weak/description-only canonical merges; #110 flags same-window multi-person visibility, but no production tracker proves same-source continuity. | #106, #109, #110 | Contract/CI + prior real run still failing | Partial | Validate same-window behavior and add stable tracker-backed athlete registry. |
| REAL-ID-004 | Single-athlete draft does not contain another visible athlete unless intentional. | #110 adds `multi_person_clip_gate`: extra visible subject IDs become `MULTI_PERSON_CLIP` review-required unless the event is explicitly `SOCIAL_MOMENT` / `HIGH_FIVE` / intentional multi-person. Not real-run validated yet. | #110 | Contract/CI only | Closed by contract | Validate against the next real run and tracker-backed visible-subject evidence. |
...
The ledger foundation from `REAL-TRACE-001` / `REAL-VALUE-001`, the first run-level athlete canonicalization guard, and the first same-window multi-person guard are now implemented by contract, so the active repair order is:

1. Add `wave_completion_score` and boundary evidence for `REAL-WAVE-002`.
2. Add operator UI gating for `REAL-UI-001`.
3. Add multi-video batch upload for `REAL-UPLOAD-001`.
4. Validate all quality claims with a real pipeline run only after audit/gap files remain synchronized.
