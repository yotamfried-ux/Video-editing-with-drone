...
- A multipart object supports at most 10,000 parts and up to 5 TiB.
...
- Detect a non-seekable provider instead of copying the entire source locally.
- Close every descriptor/channel in `finally`.
...
- [ ] Add tracked Supabase schema for multipart upload metadata and exact part ETags.
...
- [ ] Persist source URI identity, source size/fingerprint, server upload ID, part size, completed ETags, retry state, and next missing part.
...
- [ ] Restore unfinished batch membership after restart.
- [ ] Calculate readiness on the server from durable rows.
- [ ] Disable/reject pipeline start while any intended source is not size-matched `verified`.
...
- [ ] Verify final R2 size, durable part ledger, local cleanup confirmation, and absence of SportReel temporary upload files.
- [ ] Upload a 2–3 video batch and prove every intended source enters one run exactly once.

## Closure rule

GAP-013/014 are not closed by code review or CI alone. They become experiment-entry ready only after the exact deployed API, migration, native build, Android device, SD/USB provider, R2 bucket, and real large video pass the evidence gate above.